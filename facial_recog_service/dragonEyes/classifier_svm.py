...
from dragonEyes.load_embeddings import filter_dataset, split_dataset, get_dataset, get_classify_dataset
import dragonEyes.load_embeddings as lfw_input
...
def test(input_directory, model_path, classifier_output_path, batch_size, num_threads):
    prediction_set = get_classify_dataset(input_directory)
    _labels = [x.name for  x in prediction_set]

    images, labels, class_names = _load_images_and_labels(prediction_set, image_size=160, batch_size=batch_size,
                                                                num_threads=num_threads, num_epochs=1)
...
def classify(input_directory, model_path, classifier_output_path, batch_size, num_threads):
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        result = _classify(emb_array, _labels, classifier_filename)
        #print(result)
        #print('ps', prediction_set)
        for i in range(len(prediction_set)):
            _i = prediction_set[i].name
            #print('***', _i, i)
            prediction_set[i].accuracy = result[_i][1]
            prediction_set[i].person_name = result[_i][0]
            prediction_set[i].isClassified = 1

        logger.info('Completed in {} seconds'.format(time.time() - start_time))
        result = {os.path.basename(p.image_paths[0]): p for p in prediction_set}
        return result
...
def _classify(emb_array, label_array, classifier_filename):
    logger.info('Evaluating classifier on {} images'.format(len(emb_array)))
    results = []
    if not os.path.exists(classifier_filename):
        raise ValueError('Pickled classifier not found, have you trained first?')
    ...
    with open(classifier_filename, 'rb') as f:
        ...
        for i in range(len(best_class_indices)):
            print('%4d  %s: %.3f' % (i, class_names[best_class_indices[i]], best_class_probabilities[i]))
            results.append([class_names[best_class_indices[i]], best_class_probabilities[i]])

        return dict(zip(label_array, results))
...
if __name__ == '__main__':
    ...
    input_directory='data/custom'
    model_path='transferredSkills/faceNet_models/20180402-114759/20180402-114759.pb'
    classifier_output_path = 'memory/classifier.pkl'
    is_train = False
    num_threads = 16
    num_epochs = 25
    min_images_per_labels = 1

    if 0:
        main(input_directory=input_directory, model_path=model_path, classifier_output_path=classifier_output_path,
            batch_size=args.batch_size, num_threads=args.num_threads, num_epochs=args.num_epochs,
            min_images_per_labels=args.min_images_per_class, split_ratio=args.split_ratio, is_train=is_train)

    if 1:
        classify(input_directory='data/test', model_path=model_path, classifier_output_path=classifier_output_path,
