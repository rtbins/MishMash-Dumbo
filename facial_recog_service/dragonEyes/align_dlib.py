...
class AlignDlib:
    ...
    def getAllFaceBoundingBoxes(self, rgbImg, upsample_count = 2):
        """
        ...
        try:
            return self.detector(rgbImg, upsample_count)
    ...
    def _getBoxCoordinates(self, box):
        return box.top(), box.right(), box.bottom(), box.left()

    
    def _boxToRect(self, css):
        """
        tuple to dlib rect object
        """
        return dlib.rectangle(css[3], css[0], css[1], css[2])

    def faceBoundary(self, img, upsample_count=1):
        """
        ...
        """
        return self.detector(img, upsample_count)
    

    def _facePoints(self, faceImg, faceLocs=None):
        if faceLocs is None:
            faceLocs = self.getAllFaceBoundingBoxes(faceImg)
        else:
            faceLocs = [self._boxToRect(faceLoc) for faceLoc in faceLocs]


        return [self.predictor(faceImg, faceLoc) for faceLoc in faceLocs]


    def facePoints(self, faceImg, faceLocs=None):
        """
        returns face features (lips, eyes etc) given an image
        """
        features = self._facePoints(faceImg, faceLocs)
