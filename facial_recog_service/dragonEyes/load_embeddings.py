...
def get_classify_dataset(input_directory):
    dataset = []

    images = os.listdir(input_directory)
    images.sort()
    no_of_faces = len(images)
    #print('*****************')
    # these ids are wrt path
    for i in range(no_of_faces):
        if '.csv' not in os.path.join(input_directory,images[i]):
            dataset.append(ImageClass(i, [os.path.join(input_directory,images[i])]))
            #print(i, [os.path.join(input_directory,images[i])])
    #print('*****************')
    return dataset


def filter_dataset(dataset, min_images_per_label=2):
    filtered_dataset = []
...
class ImageClass():
    ...
    def detail(self):
        return self.name + ', ' + str(len(self.image_paths)) + ' images, predicted person name: ' + self.person_name + ', id: ' + self.id + ', accuracy: ' + self.accuracy

if __name__ == '__main__':
    get_classify_dataset('data/custom/Harshit')